...
| 4 | `git push` fails | Read the error class: network timeout vs auth/policy denial vs non-fast-forward | Transient network → retry; policy → you're pushing somewhere you shouldn't; non-fast-forward → remote moved | Network: retry ≤4× with 2s/4s/8s/16s backoff. Policy: stop, check the designated branch (`dara-change-control` rule 2). Non-fast-forward: fetch and reconcile — NEVER force-push | Treating every failure as retryable. Auth errors don't heal with backoff |
